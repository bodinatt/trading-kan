  ...
  return (
    <div className={`border-b ${isDark ? 'border-gray-800' : 'border-gray-200'}`}>
      ...
      >
        ...
        <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${isDark ? 'bg-gray-800 text-gray-400' : 'bg-gray-200 text-gray-500'}`}>
          {count}
        </span>
      </button>
      {isOpen && children}
...
export function AlertsPanel() {
  ...
  const selectClass = `w-full rounded px-2 py-1.5 text-xs outline-none border ${isDark ? 'bg-gray-800 border-gray-700 text-white' : 'bg-white border-gray-300 text-gray-900'}`;
  const inputClass = `w-full rounded px-2 py-1.5 text-xs outline-none border ${isDark ? 'bg-gray-800 border-gray-700 text-white' : 'bg-white border-gray-300 text-gray-900'}`;

  return (
    <div className="flex flex-col h-full">
      {/* Create Alert Section */}
      <div className={`p-3 border-b ${isDark ? 'border-gray-800' : 'border-gray-200'}`}>
        <div className={`text-xs uppercase mb-2 font-medium ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
          {t.newAlert} — {symbol}
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <button
            onClick={handleAdd}
            className="w-full px-3 py-1.5 bg-blue-600 text-white text-xs rounded hover:bg-blue-500 transition-colors"
          >
            {t.add}
          </button>
        </div>
                    ...
                    >
                      ...
                      <button
                        ...
                        className={`ml-2 flex-shrink-0 p-0.5 rounded transition-colors ${
                          isDark ? 'text-gray-600 hover:text-red-400 hover:bg-gray-800' : 'text-gray-400 hover:text-red-500 hover:bg-gray-100'
                        }`}
                        title={t.remove}
                      >
                ...
                <div>
                  {triggeredAlerts.map((alert) => (
                    <div
                      key={alert.id}
                      className={`flex items-center justify-between px-3 py-2 text-xs border-b ${
                        isDark ? 'border-gray-800/50 bg-yellow-900/10' : 'border-gray-100 bg-yellow-50'
                      }`}
                    >
                      <div className="flex items-center gap-1.5 min-w-0 flex-1">
                        {alert.category === 'indicator' && (
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-yellow-500/20 text-yellow-500 uppercase">
                              {t.triggered}
                            </span>
                          </div>
                          {alert.message && (
                  ...
                  <div className={`p-2 ${isDark ? 'border-gray-800' : 'border-gray-200'}`}>
                    <button
